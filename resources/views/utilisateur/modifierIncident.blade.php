...
@endif
<form method="POST" action="{{route('incident.modifierFournisseur',['id'=>$incident[0]->id])}}">
  @csrf
    <div class="form-group">
      <label for="exampleFormControlInput1">Nom</label>
      <input type="text" @disabled(true) class="form-control" id="exampleFormControlInput1"  name="nom" value="{{$incident[0]->nom}}">
    </div>

    <div class="form-group">
        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">type incident</label>
        <select @disabled(true) class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref" name="type">
    ...
    <div class="form-group">
        ...
        <select @disabled(true) class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref" name="intervenant">
    ...
    <div class="form-group">
      ...
      <textarea @disabled(true) class="form-control" id="exampleFormControlTextarea1" rows="3" name="commentaire" >{{$incident[0]->commentaire}}</textarea>
    </div>
    <div class="form-group">
        <label for="exampleFormControlInput1">Fournisseur</label>
        <input type="text"  class="form-control" id="exampleFormControlInput1"  name="fournisseur" value="{{$incident[0]->fournisseurs}}">
      </div>
    <div class="form-group">
        <button type="submit" class="btn btn-primary">Enregistrer</button>
        <a href="{{route('incident.util')}}" class="btn btn-danger">Annuler</a>
    </div>
  </form>
 @endsection                